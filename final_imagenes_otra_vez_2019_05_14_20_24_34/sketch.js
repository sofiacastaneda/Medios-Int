...
function preload() {
  ...
  //imágenes para las instrucciones, inicio y cierre.
   intro01 = loadImage('00inicio.png');
   intro02 = loadImage('02intro.png');
   intro03 = loadImage('03intro.png');
   intro04 = loadImage('intros04.png');
   intro05 = loadImage('05intro.png');
  cortinilla = loadImage('14cierre.png');
...
function draw() {
 ...
 image(banderausa, x4, y4, tamo4, tam4);
    ...
    //Cuadrado sensible a banderas
     push();
    noStroke();
    //stroke(66,200,244);
    //strokeWeight(2);
    noFill();
    rect(posXArea, posYArea, tamanoArea, tamanoArea);
    pop();
...
if (mouseX > 880 && mouseX < 880 + 30 && mouseY > 680 && mouseY < 680 + 30) {
 ...
 print("si");
  ...
  pop();
  image(corredor, rotationX, rotationY, tamañocorre, tamañocorre);
  ...
  stroke(0,191,200);
  strokeWeight(6);
  line(520,200,520,700);
  //líneas de los países  
  line(500,700,540,700);
  line(500,500,540,500);
  line(500,350,540,350);
  line(500,200,540,200);
  pop();
  push();
  textSize(15);
  //leyenda
  noStroke();
  fill(0,191,200);
  text("Camerún: 95 horas de trabajo ", 548,205);
  text("Estados Unidos: 80 horas de trabajo ", 548,355);
  text("Japón: 75 horas de trabajo ", 548,505);
  text("Colombia: 60 horas de trabajo ", 548,705);
  pop();
...
function mouseDragged() {
  ...
  if (dist(mouseX, mouseX, xcorre, ycorre) < tamañocorre) {

    //actualiza la posicion de la imagen con la posición del mouse
    xcorre = mouseX;
    ycorre = mouseX;
  }
  
  /*if (mouseX > posXArea && mouseX < posXArea + tamanoArea && mouseY > posYArea && mouseY < posYArea + tamanoArea) {
}*/
  
  if (dist(x1, y1, tamanoArea, tamanoArea) < 400 ) {
      textSize(50);
      text('187042',width/2, height/2);
    }
    
  if (dist(x2, y2, tamanoArea, tamanoArea) < 400 ) {
      textSize(50);
      text('93738',width/2, height/2);
    }
  
  if (dist(x3, y3, tamanoArea, tamanoArea) < 400 ) {
      textSize(50);
      text('406511',width/2, height/2);
    }
  
  if (dist(x4, y4, tamanoArea, tamanoArea) < 400 ) {
      textSize(50);
...
function profesor(){
 //variables de velocidad
  ...
  if (this.velx > 0.05) {
text('VALOR 1',width/2, height/2);
    //imagen de poco dinero
  }
  
  if (this.velx > 0.10) {
text('VALOR 2',width/2, height/2);
        //imagen de mas dinero
  }
  
  if (this.velx > 0.20) {
text('VALOR 3',width/2, height/2);
        //imagen de aun mas dinero
  }
  
  if (this.velx > 0.30) {
text('VALOR 4',width/2, height/2);
...
function deviceMoved() {
 ...
 //funcion que acelera la img del profesor
