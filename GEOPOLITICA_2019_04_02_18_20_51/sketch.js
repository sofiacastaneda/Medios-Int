...
function draw() {
  ...
  for (var l = 0; l < numUnionEuropea; l = l + 1) {
    ...
    for (var o = 0; o < britanicos.length; o = o + 1) {
        if (dist(unionEuropea[l].x, unionEuropea[l].y, britanicos[o].x, britanicos[o].y) < 10) {
          ...
          //intento 2
  for (var u = 0; u < unionEuropea.length; u = u + 1) 
   if (dist(britanicos[u].x, britanicos[u].y, unionEuropea[l].x, unionEuropea[l].y) < 10){
     britanicos[u].alejarse();
     demasiadocerca = true;
  }
  ...
  */
  ...
  for (var r = 0; r < numChinos; r = r + 1) {
   ...
   //REINO UNIDO MUERE SI SE ACERCA A CHINA
for (var w = 0; w < britanicos.length; w = w + 1) {
      if (dist(chinos[r].x, chinos[r].y, britanicos[w].x, britanicos[w].y) < 30) {
        britanicos[w].morirse();
      }
    }
