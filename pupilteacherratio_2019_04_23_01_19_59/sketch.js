...
function setup() {
  ...
  push();
    fill(255);
   textSize(18);
  text('Save this graph.', 30, 460);
    textSize(15);
  text('Data obtained from UN Data: a World of information.', 10, 480);
      textSize(10);   
    pop();
  
  createA('http://data.un.org/Data.aspx?q=colombia&d=UNESCO&f=series%3aPTRHC_23%3bref_area%3aCOL', 'link');
  ////////////////////////////////////////
 ...
 ////////////////////////////////////////
  ...
  for (var i = 0; i < filas; i = i + 1) {
    ...
    //noStroke();
    ellipse(posX+15, posY, alto, alto);
    
    //strokeWeight(3);
    fill(random(0,255),random(0,255),random(0,255));
    line(posX+15,120,posX+15,380);
    fill(0);
    
    //TEXTO INFERIOR: AÑO
     push();
    translate(posX+20, height - 100);
    rotate(-HALF_PI);
    fill(0);
    textSize(13);
    textAlign(CENTER);
  ...
  rect(posXrec1, posYrec1, tamRect1, tamRect1);
   ...
   if (mouseIsPressed == true) {
    ...
    if (mouseX > posXrec1 && mouseX < posXrec1 + tamRect1  && mouseY > posYrec1 && mouseY < posYrec1+ tamRect1) {
        saveCanvas("Pupil-teacher ratio graph", 'jpg');
      return false;
}
}
